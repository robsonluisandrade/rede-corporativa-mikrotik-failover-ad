...
| Servidor Zabbix | SRV-Zabbix (192.168.88.242) — VM Ubuntu / Hyper-V  |
...
| Função            | Controlador de Domínio (DC), DNS, Servidor de Arquivos, Hyper-V |
...
| Hyper-V           | Hospeda VM do Zabbix (Ubuntu 22.04 — IP 192.168.88.242)       |
...
| SNMP              | Habilitado — monitorado pelo Zabbix (template Synology DiskStation by SNMP) |
...
| Dispositivo       | IP             | MAC*              | Obs                                         |
| ----------------- | -------------- | ----------------- | ------------------------------------------- |
| Impressora-01     | 192.168.88.225 | AA:BB:CC:01:01:01 | Sem internet — SNMP monitorado pelo Zabbix  |
...
| Impressora-05     | 192.168.88.229 | AA:BB:CC:01:01:05 | Sem internet — SNMP monitorado pelo Zabbix  |
...
| ----------------- | -------------------------------------------------------- |
| Hostname          | SRV-Zabbix                                               |
| IP                | 192.168.88.242 — fixo (reserva DHCP por MAC)             |
| MAC*              | AA:BB:CC:00:03:01                                        |
| Sistema           | Ubuntu Server 22.04 LTS — VM Hyper-V no SRV-AD01         |
| Zabbix            | 7.0 LTS                                                  |
| Banco de dados    | PostgreSQL 16                                            |
| Frontend          | http://192.168.88.242/zabbix                             |
| Internet          | Sim — Fibra (mesma faixa de infraestrutura)              |
...
O frontend do Zabbix (`http://192.168.88.242/zabbix`) é também acessível via Tailscale de fora da rede, permitindo acompanhar alertas remotamente.
...
| ---------------------------- | -------------------------------------------------------------------------------- |
| Script de Logon (PS1)        | Copia e atualiza ERP.exe por controle de versão por data; cria atalhos por perfil |
...
| .155 — .194  | PCs do domínio          | Internet via Fibra — Zabbix Agent 2                   |
| .195 — .224  | Câmeras IP              | Sem internet — rede local apenas                      |
| .225 — .233  | Impressoras e Relógio   | Sem internet — SNMP Zabbix (Impressora-01 e 05)       |
| .234         | Servidor AD             | IP fixo no SO — Zabbix Agent 2                        |
| .235 — .253  | Infraestrutura          | NVR/NAS: Starlink — APs: Fibra — Zabbix (.242)        |

---

## 13. Monitoramento — Zabbix
...
| IP                | 192.168.88.242                                           |
| Versão            | Zabbix 7.0 LTS                                           |
| Banco de dados    | PostgreSQL 16                                            |
| Frontend          | http://192.168.88.242/zabbix                             |
| SNMP roteador     | v2c — community restrita ao .242                         |
| SNMP NAS          | v2c — community restrita ao .242                         |
| SNMP impressoras  | v2c — community public (padrão de fábrica)               |
| Agente PCs/AD     | Zabbix Agent 2 — porta TCP 10050                         |

### 13.2 Dispositivos Monitorados

| Dispositivo            | IP             | Método       | Template                        |
| ---------------------- | -------------- | ------------ | ------------------------------- |
| MikroTik hEX           | 192.168.88.1   | SNMP v2c     | MikroTik SNMP                   |
| SRV-AD01               | 192.168.88.234 | Agent 2      | Windows by Zabbix Agent         |
| NAS-Synology           | 192.168.88.235 | SNMP v2c     | Synology DiskStation by SNMP    |
| PC-Comercial           | 192.168.88.155 | Agent 2      | Windows by Zabbix Agent         |
| PC-Compras             | 192.168.88.156 | Agent 2      | Windows by Zabbix Agent         |
| PC-Faturamento         | 192.168.88.157 | Agent 2      | Windows by Zabbix Agent         |
| PC-Financeiro          | 192.168.88.158 | Agent 2      | Windows by Zabbix Agent         |
| PC-Administracao       | 192.168.88.159 | Agent 2      | Windows by Zabbix Agent         |
| PC-Laboratorio         | 192.168.88.160 | Agent 2      | Windows by Zabbix Agent         |
| PC-Qualidade           | 192.168.88.161 | Agent 2      | Windows by Zabbix Agent         |
| Impressora-01          | 192.168.88.225 | SNMP / ICMP  | Printer by SNMP                 |
| Impressora-05          | 192.168.88.229 | SNMP / ICMP  | Printer by SNMP                 |

### 13.3 Alertas Configurados

| Dispositivo / Grupo  | Condição de alerta                                          |
| -------------------- | ----------------------------------------------------------- |
| Todos os hosts       | Inacessível (sem resposta por 5 min)                        |
| Servidor AD          | CPU > 85% por 10 min / Serviço AD parado / Disco < 10 GB   |
| NAS Synology         | Disco com falha / Temperatura > 55°C / Uso > 80%           |
| PCs do domínio       | Disco < 5 GB / RAM > 90% / Agente parado                   |
| Impressoras          | Offline / Toner < 20% (SNMP)                               |
| MikroTik             | Interface WAN caída / CPU > 90% / Temperatura > 60°C       |

### 13.4 Regras de Firewall do Bloco 8

As regras abaixo são adicionadas ao MikroTik pelo script `bloco8-zabbix-monitoramento.rsc` e devem ser posicionadas antes da regra `drop` final da chain input:

| Regra                  | Chain   | Protocolo/Porta | Efeito                                         |
| ---------------------- | ------- | --------------- | ---------------------------------------------- |
| SNMP roteador          | input   | UDP 161         | Zabbix consulta o MikroTik via SNMP            |
| ICMP (ping)            | forward | ICMP            | Zabbix pinga qualquer dispositivo da rede      |
| Agente Zabbix          | forward | TCP 10050       | Zabbix coleta dados dos PCs e AD               |
| SNMP dispositivos      | forward | UDP 161         | Zabbix consulta NAS e impressoras              |

### 13.5 Verificação

```bash
# No servidor Zabbix (Ubuntu)
...
*Documentação Técnica — MikroTik hEX — 192.168.88.0/24 — Junho 2026*  
